  ...
  bindZouyueTap: function (options) {
    ...
    if ((yinlv >= 10) && (role == 'rumendizi')) {
      ...
      if (random == 1) {
        ...
        wx.redirectTo({
          url: '../../event/event?event=lingyindizi',
        })
        return
      }
    }
    ...
    if ((yinlv >= 300) && (shujia.indexOf('ningyinquxinde') == -1)) {
      ...
      wx.redirectTo({
        url: '../../event/event?event=ningyinqu',
      })
      return
    }
    ...
    if ((yinlv >= 300) && (shujia.indexOf('qiongguangpozhenquxinde') == -1)) {
      ...
      wx.setStorage({ key: 'shujia', data: shujia })
      wx.redirectTo({
        url: '../../event/event?event=qiongguangpozhenqu',
      })
      return
    }
    ...
    if (role == 'lingyindizi') {
      ...
      if (random == 1) {
        ...
        wx.redirectTo({
          url: '../../event/event?event=lingyinzhidian',
        })
        return
      }
    }
