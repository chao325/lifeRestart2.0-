...
Page({
  ...
  onShow: function() {
    ...
    if (!todaysEventHappend) {
      ...
      if (time == 2) {
        wx.redirectTo({
          url: '../fight/fight?enemy=qiecuodizi',
        })
        return
      }
      // 打架弟子
      if (time == 5) {
        wx.redirectTo({
          url: '../fight/fight?enemy=dajiadizi',
        })
        return
      }
      // 入侵前夕
      ...
      if (time == 360) {
        wx.redirectTo({
          url: '../fight/fight?enemy=smallboss1',
        })
        return
      }
      // 救活一贫
      if ((time == 360 || time == 361) && (npc.yipin == 'si') && (yaogui.indexOf('chixueliuzhudan') != -1)) {
        ...
        wx.redirectTo({
          url: '../event/event?event=yipinfuhuo',
        })
      }
      // 即将去世
      if (time == 3590) {
      ...
      if (time >= 3) {
        ...
        if (random == 1) {
          ...
          wx.redirectTo({
            url: '../fight/fight?enemy=' + randomMonster,
          })
          return
        }
      }
      ...
      if ((xiuwei >= 10) && (role == 'shushandizi')) {
        ...
        wx.redirectTo({
          url: '../event/event?event=rumendizi',
        })
        return
      }
      // 掌门入室弟子
      if ((time < 36) && (role == 'rumendizi')) {
        ...
        if (random == 1) {
          ...
          wx.setStorage({ key: 'shujia', data: shujia })
          wx.redirectTo({
            url: '../event/event?event=taiwudizi',
          })
          return
        }
      }
      ...
      if ((gongxian >= 500) && (role == 'rumendizi')) {
        ...
        wx.setStorage({ key: 'shujia', data: shujia })
        wx.redirectTo({
          url: '../event/event?event=zhanglao',
        })
        return
      }
      // 代掌门
      if ((gongxian >= 1000) && (role == 'rumendizi' || role == 'zhanglao')) {
        ...
        wx.redirectTo({
          url: '../event/event?event=daizhangmen',
        })
        return
      }
      // 武圣
      ...
      if ((xiuwei >= 3000 && jiuliang >= 1000) && (role == 'yipindizi')) {
        ...
        wx.redirectTo({
          url: '../event/event?event=jiusheng',
        })
        return
      }
      // 书圣
      if ((xiuwei >= 3000 && xueshi >= 1000) && (role == 'yushudizi')) {
        ...
        wx.redirectTo({
          url: '../event/event?event=shusheng',
        })
        return
      }
      // 笔圣
      ...
      if ((xiuwei >= 3000 && yinlv >= 1000) && (role == 'lingyindizi')) {
        ...
        wx.redirectTo({
          url: '../event/event?event=yinsheng',
        })
        return
      }
      // 铸剑长老
      if ((xiuwei >= 3000 && duanzao >= 1000) && (role == 'ziyindizi')) {
        ...
        wx.redirectTo({
          url: '../event/event?event=zhujianzhanglao',
        })
        return
      }
      // 掌门
      if ((xiuwei >= 5000) && (gongxian >= 1000) && (role != 'zhangmen')) {
        ...
        wx.redirectTo({
          url: '../event/event?event=zhangmen',
        })
        return
      }
      wx.setStorage({ key: 'todaysEventHappend', data: true })
  ...
  playBgm: function() {
    ...
    if (!that.data.bgm) {
      innerAudioContext.src = 'https://webfs.ali.kugou.com/202202131331/db460dd099649177145030b166c7b79a/KGTX/CLTX001/ab9d8d80ea1662a9a3b829e9d7c0d921.mp3'
