...
Page({
  ...
  bindFightTap: function() {
    ...
    if (enemy == 'smallboss1') {
      wx.redirectTo({
        url: '../fight/fight?enemy=smallboss2',
      })
    } else if (enemy == 'smallboss2') {
      wx.redirectTo({
        url: '../fight/fight?enemy=smallboss3',
      })
    } else if (enemy == 'smallboss3') {
      wx.setStorage({ key: 'gongxian', data: gongxian + 300 })

      wx.redirectTo({
        url: '../fight/fight?enemy=bigboss',
      })
      ...
      wx.redirectTo({
        url: '../event/event?event=bigbosswin',
      })
    } else {
      // 常规战
  ...
  bindEscapeTap: function () {
    ...
    if (enemy == 'smallboss1' || enemy == 'smallboss2' || enemy == 'smallboss3') {
      ...
      wx.redirectTo({
        url: '../event/event?event=yipinzhansi',
      })
    } else if (enemy == 'bigboss') {
      wx.setStorage({ key: 'todaysEventHappend', data: true })

      wx.redirectTo({
        url: '../event/event?event=bigboss',
      })
    } else {
      // 常规战
      wx.setStorage({ key: 'todaysEventHappend', data: true })
